...
/**
 * Creates a scope in a {@link Schedulable} object which will be
 * asynchronously interrupted at the expiration of a timer.  This timer
...
public class Timed extends AsynchronouslyInterruptedException
{
   ...
   * is called.
   * @param time When {@code time} is a {@link RelativeTime} value,
   * it is the interval of time between the invocation of
   * {@code doInterruptible} and the time when the schedulable is
   * asynchronously interrupted.  When {@code time} is an {@link
   * AbsoluteTime} value, the timer asynchronously interrupts at this
   * time (assuming the timer has not been cancelled).
   *
   * @throws StaticIllegalArgumentException when {@code time} is
   *            {@code null}.
   *
   * @throws StaticUnsupportedOperationException when {@code time} is not
   *         based on a {@link Clock}.
   */
